...
class Follow_Wall(Node):
	...
	def processIR(self):
		...
		if d1 <= 0.06:
			...
			if count == 1: 
				self.prev_wall = p[0,:2]
				
				order = p[0,2]
				print("order" + str(order))
				print("d1:" + str(chosed[0]))
				if chosed[0] > 0.06 and chosed[0] <= 0.285:
					if order == 0:
						self.move(0.3,0.15)
						self.prev_side = "L"
						self.state = "FW"
				...
				if self.state == "FW":
					if self.prev_side == "L":
						...
						if self.prev_side == "R":
							...
							self.linear_velocity_publisher.publish(twist)
							self.state = "wandering"
					# to_Wall = self.prev_wall - np.array([self.x,self.y])
					# head_vec = self.T[:2,0];
